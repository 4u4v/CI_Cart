...
{
	if (confirm('确定把该订单放入回收站吗？')){    
	  window.location.href= '<?php echo site_url('order/in_recycle/order')?>'+'/'+id;
    }
}

$(document).ready(function() {
  $('#search-action').click(function(){
   $.post('<?php echo site_url('order')?>', $('#search-form').serialize(), function(data) {
	  $('.x-grid3-row').remove();
	  $('#page').remove();
...
<div id="main-tabs" class="x-tab-panel x-border-panel ext-ie'" style="left: 5px; top: 0px; ">
 ...
 <span class="x-tab-strip-text">订单列表</span>
 </span>
 </em>
 </a>
 </li>
 ...
 <div style="position: relative;" class="x-panel-bwrap">
   ...
   <form id="search-form" style="overflow:;">
	  ...
	  订单状态 <select name='status'style="width:120px;height:22px;">
