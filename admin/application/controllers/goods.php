	...
	function update(){
		...
		/* if ($this->uri->segment(3) === FALSE) {
			...
			if ($this->form_validation->run() == false) {
				...
				$data['goods_img'] = $fileinfo['file_name'];//已上传的文件名
				
				$config_img['image_library'] = 'gd2';
				$config_img['source_image'] = "./public/uploads/".$fileinfo['file_name'];
				$config_img['create_thumb'] = true;//生成缩略图
				$config_img['maintain_ratio'] = true;
				$config_img['width'] = 160;
				$config_img['height'] = 160;
				//载入并初始化图像处理类
				$this->load->library('image_lib',$config_img);
					if ($this->image_lib->resize()) {
					// 缩略ok,得到缩略图的名称
				    $data['goods_thumb']=$fileinfo['raw_name'].$this->image_lib->thumb_marker.$fileinfo['file_ext'];
					} else {
					//缩略失败
					$title = "缩略失败";
