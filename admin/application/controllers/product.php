...
class Product extends CI_Controller {
	...
	function insert(){
		...
		if ($this->upload->do_upload('goods_img')) {
			...
			$config_img['source_image'] = "./upload/products/" . $res['file_name'];
			...
			if ($this->image_lib->resize()) {
				...
				if ($this->product_model->add_product($data)) {
					...
					$target_url = site_url("product/index");
				...
				$target_url = site_url("product/add");
	...
	public function edit($product_id){
		//获取当前这条记录的信息
		$data['current_product'] = $this->product_model->select_product($product_id);
		var_dump($data['current_product']);
	...
	function update(){
		...
		/* if ($this->uri->segment(3) === FALSE) {
			...
			//更新操作
			$this->form_validation->set_rules('product_name','分类名称','trim|required');
			if ($this->form_validation->run() == false) {
				//未通过表单验证
				$title = "未通过表单验证";
				$content = validation_errors();
				$target_url = site_url("product/edit");;
				message($title, $content, $target_url, $delay_time = 3);
			} else {
			    #上传参数配置
				...
				if($this->product_model->update_product($data,$product_id))
				{
					$title = "";
