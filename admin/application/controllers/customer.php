...
class Customer extends Controller
{
	...
	function edit()
    {
        ...
        if (!empty($params['id']) && $params['id'] > 0){
			...
			$data['header_text'] = '编辑用户(ID:'.$data['editing']['id'].')';
        } else {
            $data['editing'] = array(
                'id' => null,
                'name' => null,
	   ...
	   // 如果提交数据符合所设置的规则，则继续运行
		if (TRUE == $this->validation->run()){
            ...
            if ($id){
				...
				if($re_edit){ 
                    show_message2('"用户(ID:'.$id.')" 已保存!', 'customer/edit/id/'.$id);
			    }else{
			...
			// 添加用户
			   ...
			   if($re_edit){ 
                   ...
                   show_message2('"用户(ID:'.$newly_one['id'].')" 已保存!', 'customer/edit/id/'.$newly_one['id']);
			   }else{
				    show_message2('保存成功!', 'customer');
			    }
            }
	...
	function edit_address()
	{
        ...
        if (!empty($params['address']) && $params['address'] > 0){
			...
			$data['header_text'] = '编辑用户地址(ID:'.$data['editing']['id'].')';
        } else {
            $data['editing'] = array(
                'id' => null,
                'address_name' => null,
	...
	function save_address()
	{
		...
		if ($id){
			...
			if($re_edit){ 
				show_message2('"用户地址(ID:'.$id.')" 已保存!', 'customer/edit_address/id/'.$customer_id.'/address/'.$id);
			}else{
        ...
        // 添加新地址簿
		   ...
		   if($re_edit){ 
			   show_message2('"用户地址(ID:'.$newly_one['id'].')" 已保存!', 'customer/edit_address/id/'.$customer_id.'/address/'.$newly_one['id']);
		   }else{
				show_message2('保存成功!', 'customer/edit_address/id/'.$customer_id);
			}
		}
	...
	function set_default()
	{
        ...
        if (isset($params['address']) && ($id = $params['address']) > 0){
            ...
            if ($this->customer_address_model->set_default($id,$customer_id,1)){
               // show_message2('"用户默认地址已经更新', 'customer/edit_address/id/'.$customer_id);
            } else {
	...
	function delete()
    {
        ...
        if (isset($params['id']) && ($id = $params['id']) > 0){
            ...
            if ($this->customer_model->delete($id)){
                show_message2('"用户(ID:'.$id.')" 已被删除!', 'customer');
            } else {
                return show_message2('无效ID:'.$id, 'customer');
            }
        }
    ...
    function delete_address()
    {
        ...
        if (isset($params['address']) && ($id = $params['address']) > 0){
            ...
            if ($this->customer_address_model->delete($id)){
                //show_message2('"用户默认地址已被删除', 'customer/edit_address/id/'.$customer_id);
            } else {
               // return show_message2('无效ID:'.$id, 'customer/edit_address/id/'.$customer_id);
            }
        } 
